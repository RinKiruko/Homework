...
struct Stack *initStack(void *initValue) {
    nodeptr *newNode = initNodep(initValue);
...
void stackPut(struct Stack *stack, void *value) {
    ...
    nodeptr *nextNode = stack->next;
    nodeptr *newNode = initNodep(value);
    if (stack->len >= 1)
        chainPtrElements(newNode, nextNode);
